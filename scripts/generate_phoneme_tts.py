...
def synthesize_with_pyttsx3(text, out_path: Path):
    engine = pyttsx3.init()
...
def save_audio_safe(path: Path, waveform: torch.Tensor, sr: int):
    data = waveform.squeeze(0).numpy()
    sf.write(str(path), data, sr, subtype="PCM_16")


def resample_to_target(in_path: Path, out_path: Path, target_sr: int = TARGET_SR):
...
def main():
    ...
    for phoneme, word in phoneme_examples.items():
        ...
        # use soundfile for duration (avoids torchcodec issues)
